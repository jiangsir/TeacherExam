...
public class Utils {
	...
	public static String parsePath(String path) {
		...
		Matcher m1 = Pattern.compile("['\\\\']+").matcher(path);
		path = m1.replaceAll("/");
		Matcher m2 = Pattern.compile("['/']+").matcher(path);
		path = m2.replaceAll("/");
		if (System.getProperty("os.name").toLowerCase().startsWith("windows")) {
			path = path + "/"; // 僅確保最後一個斜線。
			Matcher m = Pattern.compile("['/']+").matcher(path);
			return m.replaceAll("\\\\");// 替換為一個斜線
		} else {
			path = "/" + path + "/"; // 確保前後都有斜線。
			Matcher m = Pattern.compile("['/']+").matcher(path);
			return m.replaceAll("/");// 替換為一個斜線
	...
	public static boolean isMANAGER_IP(String CurrentIP) {
		return Utils.isSubnetwork(
				new String[] { ENV.context.getInitParameter("MANAGER_IP") },
				CurrentIP);
	}
	 ...
	 * @return
	 */
	public static boolean isSubnetwork(String[] networkarray, String currentIP) {
		if (networkarray == null || networkarray.length == 0) {
			return false;
		}
		// if ("*".equals(networkstring)) {
		// return true;
		// }
		for (String network : networkarray) {
			if (isSubip(network, currentIP)) {
				return true;
			}
		}
		return false;
	...
	private static boolean isSubip(String ip, String currentip) {
		...
		if ("*".equals(ip) || ip.equals(currentip)) {
			return true;
		}
	 ...
	 */
	public static java.sql.Date parseDate_Deprcated(String datestring)
			throws DataException {
		...
		try {
			...
			e.printStackTrace();
			throw new DataException("日期格式(" + datestring + ")不正確！");
		}
	}
