...
public class XMLParser {
	 ...
	 */
	public XMLParser() {
		this.webxml = new File(ENV.getAPP_REAL_PATH() + "/WEB-INF/", "web.xml");
		this.contextxml = new File(ENV.getAPP_REAL_PATH() + "/META-INF/",
				"context.xml");
	}
	...
	public XMLParser(File BASE_DIR) {
		...
		System.out.println("webxml=" + this.webxml.getPath() + "/"
				+ this.webxml.getName());
		System.out.println("contextxml=" + this.contextxml.getPath() + "/"
				+ this.contextxml.getName());
	}
	...
	public void writetoWebxml() throws AccessException {
		...
		if (this.doc_webxml == null) {
			throw new AccessException(this.webxml.getPath()
					+ " doc_webxml == null 不能寫入檔案");
		}
		try {
			...
			e.printStackTrace();
			throw new AccessException(this.webxml.getPath() + " 不存在或無法讀寫。");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public TreeMap<String, String> getInitParams() {
		...
		Enumeration<?> enumeration = ENV.context.getInitParameterNames();
		while (enumeration.hasMoreElements()) {
			String name = enumeration.nextElement().toString();
			// text.append(name + " = " + ENV.context.getInitParameter(name) +
			// "<br>");
			params.put(name, ENV.context.getInitParameter(name));
		}
		return params;
	}
	...
	public TreeMap<String, String> getContextAttributes() {
		...
		Enumeration<?> enumeration = ENV.context.getAttributeNames();
		// StringBuffer text = new StringBuffer(5000);
		while (enumeration.hasMoreElements()) {
			String name = enumeration.nextElement().toString();
			// text.append(name + " = "
			// + this.getServletContext().getAttribute(name) + "<br>");
			atts.put(name, ENV.context.getAttribute(name).toString());
		}
		return atts;
	}
	...
	public void setInitParam(String param, String value) {
		...
		// FIXME 尚未實驗。要實驗是否會新增，以及新增出來是否正確
		SAXBuilder builder = new SAXBuilder();
		try {
			if (this.doc_webxml == null) {
				this.doc_webxml = builder.build(this.webxml);
