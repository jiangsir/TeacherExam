									...
									<c:if test="${exam.isactive}">
										<tr>
											<td width="60%">
												<div id="activeExam">
													<h2>${exam.title}</h2>
													<button id="disactiveExam" examid="${exam.id }" class="btn btn-danger">結束本次甄試</button>
											</td>
											<td width="50%"><a href="./EditExam.html?examid=${exam.id}" type="button"
													class="btn btn-default">修改甄試設定</a> <a href="./ManageExam.html?examid=${exam.id}" type="button"
													class="btn btn-default">管理報名人員</a> <a href="./Applications.html?examid=${exam.id}" type="button"
													class="btn btn-default">列印監試資料</a>
												<a href="./ExportCSV.api?target=exam&examid=${exam.id }" type="button"
													class="btn btn-default">匯出報考者資料(報部用) </a>
												<div id="disactiveExam_dialog" title="確定結束本次甄試？" style="display: none;">
													確定結束『${exam.title }』?</div>
												</div>
											</td>
										</tr>
							...
							<table class="table table-hover">
								<tbody>
									<c:forEach var="exam" items="${exams}">
										<c:if test="${!exam.isactive}">
											<tr>
												<td width="50%"><span style="font-size: 1.2em;">#${exam.id}.${exam.title}</span>
													<span style="margin: 3px;">
														<c:if test="${exam.isactive==false}">
															<button id="enactiveExam" examid="${exam.id }" class="btn btn-success">設定為進行中</button>
														</c:if>
														<c:if test="${exam.isactive==true}">
															進行中...
														</c:if>
													</span>
												</td>
												<td width="50%"><a href="./EditExam.html?examid=${exam.id}" type="button"
														class="btn btn-default">修改甄試設定</a> <a href="./ManageExam.html?examid=${exam.id}"
														type="button" class="btn btn-default">管理報名人員</a>
													<a href="./Applications.html?examid=${exam.id}" type="button" class="btn btn-default">列印監試資料</a>
													<a href="./ExportCSV.api?target=exam&examid=${exam.id}" type="button"
														class="btn btn-default">匯出報考者資料(報部用) </a>
													<button id="deleteExam" class="btn btn-default" data-examid="${exam.id}">刪除</button>
													<div id="deleteExam_dialog" title="確定要刪除這個甄試？" style="display: none;">
														<h3>刪除</h3>
														<h3>【#${exam.id}. ${exam.title}】?</h3>
													</div>
													<div id="enactiveExam_dialog" title="確定啓動本次甄試？" style="display: none;" class="btn btn-default">
														將會啓動『${exam.title}』，並且停止所有其它甄試。</div>
												</td>
											</tr>
										</c:if>
	 ...
	 -->
				...
				<div id="tabs-2" role="tabpanel" class="tab-pane">
					...
					<form action="" method="post" id="form">
						<table>
							...
							<tr>
								<th>系統標題：</th>
								<td><input name="title" type="text" value="${applicationScope.appConfig.title}"
										size="40" />
								</td>
							</tr>
							<tr>
							...
							<tr>
								<th scope="col">校長：</th>
								<td><input name="principal" type="text" value="${applicationScope.appConfig.principal}"
										size="20" /></td>
							</tr>
							<tr>
								<th scope="col">主辦主計：</th>
								<td><input name="zhuji" type="text" value="${applicationScope.appConfig.zhuji}"
										size="20" />
								</td>
							</tr>
							<tr>
								<th scope="col">出納：</th>
								<td><input name="chuna" type="text" value="${applicationScope.appConfig.chuna}"
										size="20" />
								</td>
							</tr>
							<tr>
								<th scope="col">人事室：</th>
								<td><input name="renshi" type="text" value="${applicationScope.appConfig.renshi}"
										size="20" /></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td><input name="bankprefix" type="text"
										value="${applicationScope.appConfig.bankprefix}"
										size="20" />(應向台灣銀行申請虛擬帳號才會獲得此編號，本系統僅支援產生台灣銀行虛擬帳號)</td>
							</tr>
							<tr>
								<th scope="col">台灣銀行及分行：</th>
								<td><input name="bankname" type="text" value="${applicationScope.appConfig.bankname}"
										size="40" />(e.q
									台灣銀行 高雄分行)</td>
							</tr>
							<tr>
								...
								<td>
									<div style="font-style: italic;">${receiveDataUrl}</div>
									請將此網址回報台灣銀行電子金融部。<br />本接收程式僅適用於接收台灣銀行電子對帳資料(各銀行有各自對帳資料格式)，若收款銀行並非台灣銀行，則需進行人工對帳，及手動設定考生是否已繳款。
								</td>
							</tr>
						</table>
						<div>
				...
				<div id="tabs-3" role="tabpanel" class="tab-pane">
					...
					<c:forEach var="user" items="${users}">
						<div>
							${user.id}. ${user.pid}: <a href="EditUser?userid=${user.id}" title="編輯帳號"
								class="btn btn-primary">更改密碼</a>
							<!-- 
