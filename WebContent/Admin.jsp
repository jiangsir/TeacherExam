							...
							<table class="table table-hover">
								<c:forEach var="exam" items="${exams}">
									<c:if test="${exam.isactive}">
										<tr>
											<td><div id="activeExam">
													<h2>${exam.title}</h2>
													<button id="disactiveExam" examid="${exam.id }"
														class="btn btn-danger">結束本次甄試</button>
													<br /> <a href="./EditExam.html?examid=${exam.id}"
														type="button" class="btn btn-default">修改甄試設定</a> <a
						...
						<table class="table table-hover">
							<tbody>
								<c:forEach var="exam" items="${exams}">
									<c:if test="${!exam.isactive}">
										<tr>
											<td>管理 ${exam.title } <span style="margin: 3px;">
                            ...
                            進行中...
											...
											</span><br /> <a href="./EditExam.html?examid=${exam.id}"
												type="button" class="btn btn-default">修改甄試設定</a> <a
												href="./ManageExam.html?examid=${exam.id}" type="button"
							...
							<table>
								...
								<tr>
									...
									<td><input name="manager_ip" type="text"
										value="${applicationScope.appConfig.manager_ip}" size="50" />例:[0.0.0.0/0] 全部開放 或 [127.0.0.1, 192.168.1.0/24] 開放本機與 192.168.1.* 網段</td>
								</tr>
								<tr>
									<th scope="col">可以進入網站的 IP：</th>
									<td><input name="allowed_ip" type="text"
										value="${applicationScope.appConfig.allowed_ip}" size="50" />例:[0.0.0.0/0] 全部開放 或 [127.0.0.1, 192.168.1.0/24] 開放本機與 192.168.1.* 網段</td>
								</tr>
								<tr>
