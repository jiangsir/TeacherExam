		...
		<div class="row">
			...
			<div class="noPrint">
				...
				<c:forEach var="subject" items="${subjects}">
					<a
						href="Applications.html?examid=${exam.id}&subjectid=${subject.id}"
			...
			<div>
				<c:forEach var="application" items="${applications}"
					varStatus="varstatus">
					<span class="box">${varstatus.count}<img
						src="./ShowPicture?id=${application.pictureid}"
						style="width: 100px;" /> <br />${application.pid}<br />${application.subject.zhengshidaili}${application.subject.nianduan}${application.subject.name
								}<br />
						${application.name}<br /> <c:if test="${application.seatid==''}">
								准考證號尚未製作！
       </c:if> <c:if test="${application.seatid!=''}">
	   准考証號：${application.seatid}</c:if></span>
				</c:forEach>
			</div>
