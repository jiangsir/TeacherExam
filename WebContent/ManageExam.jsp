...
<script type="text/javascript"
	src="ManageExam.js?${applicationScope.built }"></script>
<script type="text/javascript">
...
<body>
	<div class="container">
		<div class="row">
			...
			<div>
				<span id="makeSeatid" data-examid="${exam.id }"
					class="btn btn-primary">製作准考證號碼。</span>
				<div id="makeSeatid_dialog" title="確定製作准考證號碼？"
					style="display: none;">
					請注意: <br /> 1. 繳款截止之後，才能開始製作准考證。<br>2.已繳款考生才會配給准考証號碼。<br /> <br />確定要製作准考證字號？
				</div>
				<span id="setExamformprintable" data-examid="${exam.id}"
					class="btn btn-primary">開放考生列印准考證。</span>
				<div id="setExamformprintable_dialog" title="請先確定已經將准考證字號製作完畢！"
					class="btn btn-default" style="display: none;">
					請先確定已經將准考證字號製作完畢！<br />再開放考生列印准考證！
				</div>
				<hr />
				<a href="./ExportCSV.api?target=PaidXLS&examid=${exam.id}"
					type="button" class="btn btn-primary">匯出所有已繳費者 XLS 檔</a> <a
					href="./ExportCSV.api?target=PaidCSV&examid=${exam.id}"
					type="button" class="btn btn-primary">匯出所有已繳費者 CSV 檔</a> <a
					href="./ImportScorecsv?examid=${exam.id}" type="button"
					class="btn btn-primary">匯入成績表</a> <a
					href="./ExportCSV.api?target=exam&examid=${exam.id }" type="button"
					class="btn btn-primary">匯出報考者資料(報部用) </a>
				<div id="exportPaid_dialog" title="匯出已繳費的考生名冊！"
					style="display: none;" class="btn btn-default">匯出已繳費的考生名冊！</div>


			</div>
			<br />
			<ul class="nav nav-tabs" role="tablist">
				<li><a href="?examid=${exam.id}#tab_0" aria-controls="tab_0"
					role="presentation"><span class="glyphicon glyphicon-tag"></span>所有科目報名人</a></li>
				<li><a href="?examid=${exam.id}&ispaid=1#tab_1"
					aria-controls="tab_1" role="presentation"><span
						class="glyphicon glyphicon-tag"></span>列出所有已繳費者</a></li>
				<c:forEach var="subject" items="${subjects}" varStatus="status">
					<li><a
						href="?examid=${exam.id}&subjectid=${subject.id}#tab_${status.count+1}"
						role="presentation" aria-controls="tab_${status.count+1}"> <span
					...
					<c:forEach var="application" items="${applications}"
						...
						<tr>
							...
							<img
								...
								<a href="EditApplication.html?applicationid=${application.id}">${application.name}</a><br />
								<div>${application.pid}</div>
								<div>${application.bankaccount}</div> <c:if
									test="${application.seatid==''}">
								准考證號尚未製作！
       </c:if> <c:if test="${application.seatid!=''}">
	   准考証號：${application.seatid}</c:if> <br />
							...
							<td><a
								...
								</c:if> <c:if test="${!empty scorecsv.getStep1()}">
									<table class="table table-hover">
										<thead>
											<tr>
												<c:forEach var="col"
								...
								</c:if> <c:if test="${!empty scorecsv.getStep2()}">
									<table class="table table-hover">
										<thead>
											<tr>
												<c:forEach var="col"
							...
							<td><c:if test="${!application.ispaid}">
									...
									<div id="ispaid-dialog-${application.bankaccount}"
										title="確定要手動設定為已繳費？" style="display: none;">
								...
								</c:if> <c:if test="${application.ispaid}">
									<div class="input-group-btn">
										<c:set var="bankdata" value="${application.bankdata}"></c:set>
										<button class="btn btn-success" id="bankdata"
											title="${application.bankaccount}繳費明細">已繳費！</button>
										...
										<div id="unpaid-dialog-${application.bankaccount}"
											title="確定要手動取消繳費資訊？" style="display: none;">
											姓名：<span id="name">${application.name }</span><br /> 身份證字號：<span
												id="pid">${application.pid }</span><br /> 虛擬帳號：<span
												id="bankaccount">${application.bankaccount }</span>
										</div>
									</div>
									<c:if test="${application.bankdata==null}">查無繳費明細</c:if>
									<!-- 									<c:if test="${application.bankdata!=null}">
										<c:set var="bankdata" value="${application.bankdata}"></c:set>
										<button id="bankdata" class="btn btn-default">繳費明細</button>
										<div id="bankdata-dialog" title="${bankaccount} 繳費明細！"
											style="display: none;">
											虛擬帳戶：${bankdata.bot.RCPTId}<br /> 交易日期：${bankdata.bot.trnDt}<br />
											繳款時間：${bankdata.bot.trnTime}<br />
											繳費金額：${bankdata.bot.curAmt}<br /> 代收類別：${bankdata.bot.code}<br />
								...
								<div id="deleteApplication-dialog" title="確定要刪除這個報名人？"
									style="display: none;">
									姓名：<span id="name">${application.name }</span><br /> 身份證字號：<span
										id="pid">${application.pid }</span><br /> 虛擬帳號：<span
										id="bankaccount">${application.bankaccount }</span><br />
			...
			<c:if test="${fn:length(bankdatas) == 0 }">
