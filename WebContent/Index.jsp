							...
							<div class="col-md-6">
								<ul id="files">
									<c:forEach var="upfile" items="${activeExam.upfiles }">
										<li><a href="Download?upfileid=${upfile.id}" class="btn btn-default btn-lg btn-block"
												type="button">${upfile.descript}</a></li>
									</c:forEach>
								</ul>
							</div>
							<div class="clearfix"></div>
							<c:if test="${activeExam.isApplyRunning()}">
								<c:set var="exam" value="${activeExam}" scope="request" />
								<jsp:include page="include/DIV_ExamStatistic.jsp" />
								<br />
								<a href="Apply.html?examid=${activeExam.id}" type="button" class="btn btn-primary btn-lg">我確定要報名！</a>
								<a href="Personal.html?examid=${activeExam.id}" type="button"
									class="btn btn-primary btn-lg">已經完成報名，登入個人頁面</a>
							</c:if>
							<c:if test="${activeExam.Before_Applybegin()}">
								<div class="btn btn-default btn-lg">目前報名尚未開始!</div>
							</c:if>
							<c:if test="${activeExam.After_Applyend()}">
								<a href="Personal.html?examid=${activeExam.id}" type="button"
									class="btn btn-primary btn-lg">已經完成報名，登入個人頁面</a>
							</c:if>
						</td>
